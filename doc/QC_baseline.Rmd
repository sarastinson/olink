...
CVD_bridge <- OlinkAnalyze::olink_normalization(df1 = CVD_npx_df1,
                      df2 = CVD_npx_df2,
...
## Filter QC_Warning, CONTROL_SAMPLE, MissingFreq
CVD_LOD$MissingFreq <- as.numeric(CVD_LOD$MissingFreq) # convert from chr to num
str(CVD_LOD$MissingFreq)

table(CVD_LOD$MissingFreq)

CVD_long <- CVD_LOD %>%
dplyr::filter(QC_Warning != "Warning") %>%
  dplyr::filter(!str_detect(SampleID, 'CONTROL_SAMPLE')) %>%
  dplyr::filter(MissingFreq < 0.30)
...
INF_bridge <- OlinkAnalyze::olink_normalization(df1 = INF_npx_df1,
                      df2 = INF_npx_df2,
                      overlapping_samples_df1 = overlap_samples,
                      df1_project_nr = '20191262',
                      df2_project_nr = '20202245',
...
## Filter QC_Warning, CONTROL_SAMPLE, MissingFreq
INF_LOD$MissingFreq <- as.numeric(INF_LOD$MissingFreq) # convert from chr to num
table(INF_LOD$MissingFreq)
...
merge_rename <- merge_panels %>% 
  ...
  dplyr::select(-CXCL1.y, -SCF.y, -IL18.y, -FGF21.y, -CCL3.y) %>% # Remove proteins from INF panel (higher MissingFreq)
  dplyr::rename(CXCL1 = CXCL1.x,
                SCF = SCF.x,
                IL18 = IL18.x,
                FGF21 = FGF21.x,
                CCL3 = CCL3.x)
...
## Save QC'd Data
write.table(olink_df, file = here::here("data/olink_CVDII_INF_baseline_plasma.txt"), 
            sep = "\t", quote = F)
