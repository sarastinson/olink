...
CVD_wide <- CVD_qc %>%
  dplyr::select(SampleID, Assay, PlateID, corrected_NPX, QC_Warning) %>%
...
olink_df <- INF_wide %>%
  dplyr::select(-IL6, - CXCL1, -SCF, -IL18, -FGF21, -CCL3) %>% # Remove 6 duplicate proteins from INF panel
...
metadata <- INF_qc %>%
  dplyr::full_join(CVD_qc) %>%
  dplyr::group_by(Assay) %>%
  dplyr::select(UniProt, OlinkID, LOD, Panel, Panel_Version, MissingProp) %>% 
